...
async function load_posts(){
  ...
  let posts = await microblog.posts(0);//input 0 means all posts
...
async function load_follows(){
  ...
  //console.log("follownames.length="+follownames.length);
  ...
  for (var i=0; i<follownames.length; i++){
    ...
    //console.log(followname.innerText);
    //add click function for every followee button
    //console.log("pass");
    followname.onclick = async function(){
      allposts_section.replaceChildren([]);
      //get allposts of the specific followee and store it in allposts
      ...
      //console.log("followname: "+followname.innerText);
