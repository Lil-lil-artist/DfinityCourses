...
module {
  ...
  //store [A] rather than A
  public type Buckets<A> = Deque.Deque<[A]>;
  
  //List:  singly-linked lists. https://smartcontracts.org/docs/current/references/motoko-ref/list
  ...
  public class Logger<A>(s : State<A>) {
    ...
    // Add a set of messages to the log. If reach size cap, roll over.
    ...
    public func stats() : Stats {
      ...
      //get every bucket's size by popback buckets
      label LOOP loop {
    ...
    //From an external perspective, users do not know the buckets structure,
    //They perceive this as an array 
